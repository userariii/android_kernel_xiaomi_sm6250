...
function cloneTC() {
	...
	then
	post_msg " Cloning Cosmic Clang 16 ToolChain "
	...
	then
        post_msg " Cloning Aosp Clang 15.0.3 ToolChain "
        mkdir aosp-clang
        cd aosp-clang || exit
	wget -q https://android.googlesource.com/platform/prebuilts/clang/host/linux-x86/+archive/refs/heads/master/clang-r468909b.tar.gz
        tar -xf clang*
        cd .. || exit
